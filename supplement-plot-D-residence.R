...
# Load data
data <- read_csv('data/residence.csv')
...
data %<>%
    mutate(residence = factor(residence),
           residence = fct_reorder(residence,
                                   estimate_chronic))
...
pp <- ggplot(data = data) +
    ...
    geom_text(aes(y = 30,
              ...
              size = 6) +
    labs(title = 'D: Prevalence by place of residence',
...
# Save
ggsave(filename = 'figures/supplement-1-D-residence.png',
       plot = pp,
       height = 8,
       width = 8)
