...
# Process data
data %<>%
    mutate(grant = factor(grant),
           grant = fct_reorder(grant,
                               estimate_chronic))
...
pp <- ggplot(data = data) +
    ...
    geom_text(aes(y = 40,
              ...
              size = 6) +
    labs(title = 'I: Prevalence by government grant support',
         subtitle = 'Numbers in parentheses show the unweighted sample sizes',
         x = 'Receiving a social grant',
...
# Save
ggsave(filename = 'figures/supplement-1-I-grant.png',
       plot = pp,
       height = 8,
       width = 8)
